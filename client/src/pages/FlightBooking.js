...
const FlightBooking = () => {
    ...
    const calculateFlightPrice = () => {
        let basePrice = 5000;
        let classMultiplier = 1;
        if (formData.flightClass === 'Business') classMultiplier = 2;
        if (formData.flightClass === 'First Class') classMultiplier = 3;
        let total = formData.passengers * basePrice * classMultiplier;
        if (isRoundTrip) {
            total *= 2;
        }
    ...
    const handleNextStep = () => {
        ...
        if (currentStep === 1) {
            ...
            if (formData.passengers > 0 && formData.flightClass) {
                setCurrentStep(currentStep + 1);
            } else {
                setMessage('Please specify passengers and class.');
            }
    ...
    const handleSubmit = async () => {
        ...
        try {
            const totalPrice = calculateFlightPrice();
            const bookingData = {
                // Ensure tripType is always included
                tripType: isRoundTrip ? 'Round Trip' : 'One Way',
                from: formData.from,
                to: formData.to,
                departureDate: formData.departureDate,
                returnDate: isRoundTrip ? formData.returnDate : undefined,
                ...
                {currentStep === 4 && (
                    <div className="form-step-card animated-fade-in">
                        ...
                        <div className="form-group">
                            ...
                            <select name="flightClass" value={formData.flightClass} onChange={handleChange} required>
                                <option value="Economy">Economy</option>
                                <option value="Business">Business</option>
                                <option value="First Class">First Class</option>
                            </select>

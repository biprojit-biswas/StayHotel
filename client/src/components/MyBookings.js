...
//                 ...bicycleRentalsResponse.data.map(b => ({ ...b, type: 'bicycle' })), // Add bicycle rentals
//             ];
...
//     const handleCancelBicycleRental = async (rentalId) => { // New cancel function
//         const token = localStorage.getItem('token');
...
//                                         {/* You can add a cancel button here if you implement a DELETE route for flights */}
//                                     </>
//                                 )}
//                             </div>
...
import ReviewForm from './ReviewForm';
import TransferReviewForm from './TransferReviewForm';
import FlightReviewForm from './FlightReviewForm'; // Import new form
import '../App.css';
...
const MyBookings = () => {
    ...
    const [roomReviews, setRoomReviews] = useState({});
    const [transferReviews, setTransferReviews] = useState({});
    const [flightReviews, setFlightReviews] = useState({}); // New state for flight reviews
    const [showRoomReviewForm, setShowRoomReviewForm] = useState(null);
    const [showTransferReviewForm, setShowTransferReviewForm] = useState(null);
    const [showFlightReviewForm, setShowFlightReviewForm] = useState(null); // New state for form
    const [isPopupVisible, setIsPopupVisible] = useState(false);
    const [popupMessage, setPopupMessage] = useState('');
    ...
    const fetchMyBookings = async () => {
        ...
        try {
            ...
            await fetchRoomReviews(roomBookingsResponse.data);
            await fetchTransferReviews(transferBookingsResponse.data);
            await fetchFlightReviews(flightBookingsResponse.data); // Fetch flight reviews
        } catch (err) {
            setError('Failed to fetch bookings.');
            console.error('Error fetching bookings:', err);
        } finally {
    ...
    const handleCancelFlightBooking = async (flightId) => { // New cancel function
        const token = localStorage.getItem('token');
